 ...
 * CMDARGS: 20
 * CMDDESC: owners you or other users on channel(s).
 * CMDFUNC: csc_doowner
 * CMDPROTO: int csc_doowner(void *source, int cargc, char **cargv);
 * CMDHELP: Usage: owner [<channel> [<user1> [<user2> [...]]]]
 * CMDHELP: owners you on one or more channels, or owners other named users on a given channel.
 * CMDHELP: This command cannot be used to owner users who are otherwise prevented from getting
 * CMDHELP: owners, e.g. via the +d chanlev flag (see CHANLEV) or bitch mode (see CHANFLAGS).
 * CMDHELP: Where:
 * CMDHELP: channel - channel to use.  If no channel is specified, you will be given owners on
 * CMDHELP:           every channel where you have appropriate access and are not already
 * CMDHELP:           opped.
 * CMDHELP: user<n> - user(s) to owner instead of yourself.  Each user must be specified as the
 * CMDHELP:           current nickname of a user on the named channel.
 * CMDHELP: owner requires operator access (+m) on the named channel.  If this command is used
 * CMDHELP: to owner other users, a notice will be sent to other channel operators on the 
...
int csc_doowner(void *source, int cargc, char **cargv) {
  ...
  if (cargc==0) {
	...
	if (!(*(getnumerichandlefromchanhash(ca[i]->users, sender->numeric)) & 
	  ...
	  if ((rcup=findreguseronchannel(rcp, rup)) && CUHasOpPriv(rcup) && 
	    ...
	    localdosetmode_nick(&changes, sender, MC_OWNER);
  ...
  if (!(cip=cs_checkaccess(sender, cargv[0], CA_MASTERPRIV, NULL, "owner", 0, 0)))
    return CMD_ERROR;
  ...
  if (cargc==1) {
    /* Only one arg: "op me" */
    if (!cs_checkaccess(sender, NULL, CA_OWNERPRIV | CA_DEOWNER, cip, "owner", 0, 0))
      return CMD_ERROR;
  ...
  if (!cs_checkaccess(sender, NULL, CA_MASTERPRIV, cip, "owner", 0, 1))
    donotice=1;
